    ...
    if ($action === 'upload_picture') {
        if (!empty($_FILES['profile_picture']['name']) && $_FILES['profile_picture']['error'] !== UPLOAD_ERR_NO_FILE) {
            ...
            if (!$avatarResult['success']) {
                ...
                try {
                    $updated = $userModel->updateProfile($riderId, ['profile_picture' => $avatarResult['relative_path']]);
                    if ($updated) {
                        delete_old_avatar($rider['profile_picture'] ?? null);
        ...
        <div class="col-lg-4">
            ...
            <div class="card border-0 shadow-sm text-center p-4 rounded-3 mb-4" data-aos="fade-right">
                <div class="mx-auto mb-3">
                    <?php if (!empty($rider['profile_picture']) && is_file(dirname(__DIR__, 2) . '/' . ltrim($rider['profile_picture'], '/'))): ?>
                        <img src="<?= e(url($rider['profile_picture'])) ?>"
                             alt="Profile picture of <?= e($rider['full_name'] ?? 'Rider') ?>"
                ...
                <form action="<?= url('pages/rider/profile.php') ?>" method="POST" enctype="multipart/form-data" class="mb-3" id="riderAvatarForm">
                    <?= csrf_input() ?>
                    ...
                    <button type="button" class="btn btn-sm btn-primary fw-semibold px-3" id="riderAvatarBtn">
